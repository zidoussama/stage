        ...
        <div key={product._id} className="group bg-gray-50 rounded-xl p-4 flex flex-col sm:flex-row gap-6 relative">
          <div className="relative w-full sm:w-1/4 flex-shrink-0">
            <div className="bg-white rounded-lg p-4 h-full flex items-center justify-center">
              <Image onClick={() => goToProduct(product)} src={product.imageUrls[0]} alt={product.name} width={200} height={200} className="w-full h-auto object-contain rounded-md" />
            </div>
          </div>
          ...
          <div className="flex flex-col justify-between items-center sm:items-end flex-shrink-0">
            ...
            <button onClick={() => handleAddToBag(product)} className="flex items-center gap-2 bg-gray-800 text-white font-semibold py-3 px-6 rounded-lg hover:bg-gray-900 transition-colors">
