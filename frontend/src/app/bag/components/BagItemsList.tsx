                ...
                <ul className="-my-8 divide-y divide-gray-200">
                    {items.map((item) => (
                        <li key={item._id} className="flex flex-col space-y-3 py-6 text-left sm:flex-row sm:space-x-5 sm:space-y-0">
                            <div className="shrink-0"><Image className="h-24 w-24 max-w-full rounded-lg object-contain" src={item.imageUrl} alt={item.name} width={96} height={96} /></div>
                            <div className="relative flex flex-1 flex-col justify-between">
                                ...
                                <div className="absolute top-0 right-0 flex sm:bottom-0 sm:top-auto"><button type="button" className="flex rounded p-2 text-center text-gray-500 transition-all duration-200 ease-in-out hover:text-gray-900" onClick={() => handleRemoveItem(item._id)}><FaTrash className="h-5 w-5" /></button></div>
                            </div>
                        </li>
                    ))}
                </ul>
